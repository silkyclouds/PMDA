...
export default function LibraryLayout() {
  ...
  const { data: scanProgress } = useQuery<api.ScanProgress>({
    queryKey: ['library-live-scan-progress'],
    queryFn: api.getScanProgress,
    refetchInterval: (query) => {
      const progress = query.state.data;
      if (progress?.scanning || progress?.scan_starting || progress?.background_enrichment_running || progress?.status === 'running') {
        return 3000;
      }
      return 10000;
    },
  ...
  const visibleScopes = useMemo<api.LibraryBrowseScope[]>(() => {
    ...
    const wantsConsolidated = (mode !== 'inplace' && mode !== 'audit') || hasIntake;
    if (valid.length > 0) {
    ...
    const base: api.LibraryBrowseScope[] = (mode === 'inplace' || mode === 'audit')
  ...
  const libraryLiveActive = Boolean(
  ...
  const liveScopeCounts = useMemo(() => {
    if (!libraryLiveActive || scope !== 'library') return scopeMeta.counts;
    return {
      artists: Number(scanProgress?.library_visible_artists_count ?? scopeMeta.counts.artists ?? 0),
      albums: Number(scanProgress?.library_visible_albums_count ?? scanProgress?.scan_published_albums_count ?? scopeMeta.counts.albums ?? 0),
      tracks: Number(scopeMeta.counts.tracks ?? 0),
    };
  }, [libraryLiveActive, scanProgress?.library_visible_albums_count, scanProgress?.library_visible_artists_count, scanProgress?.scan_published_albums_count, scope, scopeMeta.counts]);
    ...
    <>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-2 text-sm">
            ...
            <span className="text-xs text-muted-foreground">
              {Number(liveScopeCounts.albums).toLocaleString()} albums
              {' · '}
              {Number(liveScopeCounts.artists).toLocaleString()} artists
              <span className="hidden md:inline">
                {' · '}
                {Number(liveScopeCounts.tracks).toLocaleString()} tracks
              </span>
            </span>
          </div>
      ...
      {auditMode ? (
        <div className="px-4 md:px-6 pb-3">
          <div className="flex items-start gap-3 rounded-2xl border border-amber-500/20 bg-amber-500/10 px-4 py-3 text-sm text-amber-50">
            ...
            <div>
              <div className="font-semibold text-amber-100">Audit mode — no automatic changes</div>
              <p className="mt-1 text-xs leading-5 text-amber-50/85">
                PMDA can scan, index and enrich the database view of your library, but it will not move files, rewrite full tags, publish a clean library or refresh players automatically in this workflow.
              </p>
            </div>
          </div>
        </div>
      ) : null}
      <Outlet context={{ includeUnmatched, scope, setScope, stats, statsByScope, statsReady, libraryIsEmpty, visibleScopes, emptyState, scanProgress: scanProgress ?? null, libraryLiveActive }} />
    </>
  );
}
