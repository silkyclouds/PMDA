...
export function AudioPlayer({
  ...
  const mediaSessionArtworkUrl = useMemo(
    () => api.appendPmdaAuthToken(displayAlbumThumb),
    [displayAlbumThumb],
  );
  const releaseYear = (() => {
  ...
  const assignTrackToDeck = useCallback((deckIndex: DeckIndex, track: TrackInfo | null) => {
    ...
    const trackUrl = api.appendPmdaAuthToken(track.file_url);
    if (!trackUrl) return;
    if (el.src !== trackUrl) {
      el.src = trackUrl;
      el.load();
  ...
  useEffect(() => {
    ...
    const artworkSrc = mediaSessionArtworkUrl;
    try {
                  ...
                  <div className={cn('relative aspect-square overflow-hidden rounded-3xl border shadow-[0_20px_72px_rgba(0,0,0,0.28)]', isMobile ? 'w-[min(64vw,300px)] self-center' : 'w-[min(82vw,520px)]', 'border-border/60 bg-card')}>
                    {displayAlbumThumb ? (
                      <AuthenticatedImage
                        src={displayAlbumThumb}
                        alt=""
                        className="absolute inset-0 h-full w-full animate-in object-cover fade-in-0 duration-300"
                      />
                    ) : (
