...
const PHASE_META: Record<string, PhaseMeta> = {
  ...
  media_prepare: {
    label: 'Preparing lightweight artwork links',
    description: 'PMDA is preserving available artwork references without blocking the library snapshot on cache generation.',
    progress: 76,
    tone: 'progress',
  },
  writing: {
    label: 'Updating the browse index',
    description: 'PMDA is writing artists, albums, and tracks into the visible library index.',
    progress: 86,
    tone: 'progress',
  },
  embeddings: {
  ...
  media_cache: {
    label: 'Scheduling media cache',
    description: 'PMDA is scheduling artwork cache work after the library snapshot is usable.',
    progress: 99,
    tone: 'progress',
  },
  done: {
    label: 'Finalizing library index',
    description: 'PMDA has finished the index work and is settling the final status.',
    progress: 100,
    tone: 'progress',
  },
  error: {
...
function resolvePhaseMeta(status: LibraryFilesIndexStatus): PhaseMeta {
  ...
  const base = PHASE_META[key] || {
    ...
    description: 'PMDA is refreshing the visible library index.',
  ...
  if ((key === 'discovering' || key === 'collapsing') && ratio != null) {
    return {
      ...base,
      progress: Math.max(base.progress, Math.min(key === 'discovering' ? 30 : 52, 8 + (ratio * 22))),
    };
  }
...
export function LibraryIndexStatusBanner({ status, libraryRoot }: LibraryIndexStatusBannerProps) {
  ...
  const currentArtist = String(resolvedStatus.current_artist || '').trim();
  ...
  const discoveredFolderGroups = Math.max(0, Number(resolvedStatus.discovered_folder_groups || 0));
  const ratePerSecond = Number(resolvedStatus.phase_rate_per_sec || 0);
  const etaLabel = formatEta(resolvedStatus.phase_eta_seconds);
  const phaseItemDone = Math.max(0, Number(resolvedStatus.phase_item_done || 0));
  const phaseItemTotal = Math.max(0, Number(resolvedStatus.phase_item_total || 0));
  const phaseItemLabel = String(resolvedStatus.phase_item_label || '').trim();
  ...
  const phaseSummary = phaseItemTotal > 0 && phaseItemLabel
    ? `${formatCompactCount(phaseItemDone)} / ${formatCompactCount(phaseItemTotal)} ${phaseItemLabel}`
    : '';
  const rootLabel = formatLibraryRootLabel(libraryRoot);
  const phaseMessage = String(resolvedStatus.phase_message || '').trim();
  const currentContextLabel = phaseKey === 'artist_enrichment' ? 'Current artist' : 'Current folder';
  const currentContextValue = phaseKey === 'artist_enrichment'
    ? currentArtist
    : currentFolder;
      ...
      <div className="px-4 py-3 md:px-5 md:py-4">
          ...
          <div className="min-w-0 flex-1 space-y-2">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline" className="border-primary/30 bg-primary/10 text-primary">
                {running ? 'Library index refresh' : 'Library index error'}
              </Badge>
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                {running
                  ? 'The scan can already be complete. Browse pages keep using the last published snapshot while this background index job finishes.'
                  : (resolvedStatus.error || 'PMDA could not finish refreshing the library index.')}
              </p>
            </div>
          </div>
        ...
        <div className="mt-3 space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
            ...
            {phaseKey === 'collapsing' && discoveredFolderGroups > 0 ? (
              <span>
                {formatCompactCount(discoveredFolderGroups)} discovered album folder groups
              </span>
            ) : null}
            {phaseKey === 'collapsing' ? (
              <span>
        ...
        {currentContextValue ? (
          <div className="mt-3 rounded-xl border border-border/60 bg-background/55 px-3 py-2 text-xs text-muted-foreground">
            <span className="font-medium text-foreground">{currentContextLabel}</span>
            <span className="mx-2 text-border">·</span>
            <span className="break-all">{currentContextValue}</span>
          </div>
        ) : null}
      </div>
    </div>
