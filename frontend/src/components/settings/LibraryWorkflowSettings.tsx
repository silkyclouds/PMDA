...
const WORKFLOW_CARDS: Array<{
  ...
  },
  {
    value: 'audit',
    label: 'Audit only',
    description: 'Scan and analyze the current library in read-only mode, without automatic file moves, tagging, export, or player refresh.',
    scenario: 'I want PMDA to surface dupes, incompletes and metadata gaps first, then decide on manual actions later.',
  },
  {
    value: 'custom',
...
export function LibraryWorkflowSettings({ config, updateConfig, onSwitchToCustom }: Props) {
  ...
  const effectiveScanRoots = mode === 'managed'
    ? intakeRoots
    : Array.from(new Set([...sourceRoots, ...((mode === 'inplace' || mode === 'audit') ? intakeRoots : [])]));
  ...
  return (
    <Card id="settings-library-workflow" className="scroll-mt-24 border-success/20 bg-success/[0.04]">
      <CardHeader>
        ...
        <CardDescription>
          Choose how PMDA should interpret your folders: intake + clean library, mirrored library, organize in place, or audit an existing library read-only.
        </CardDescription>
      </CardHeader>
      <CardContent className="space-y-5">
        ...
        {(mode === 'inplace' || mode === 'audit') ? (
          <>
            <PathListEditor
              label={auditMode ? 'Library folders to audit' : 'Library folders'}
              description={auditMode
                ? 'PMDA reads these folders, builds the visible library view, and surfaces dupes / incompletes without changing files automatically.'
                : 'These folders are both the scan source and the serving library. PMDA organizes them in place.'}
              paths={sourceRoots}
              onChange={(next) => setPaths('LIBRARY_SOURCE_ROOTS', next)}
              placeholder="/music/Music_matched"
              selectLabel={auditMode ? 'Select library folder to audit' : 'Select library folder'}
            />
            <PathListEditor
              label="Optional intake folders"
              description={auditMode
                ? 'Optional. PMDA can still watch intake folders separately, but audit mode keeps everything read-only until an admin triggers a manual action.'
                : 'If you also want a holding area for new arrivals, PMDA will keep those albums in Inbox until they are promoted.'}
              paths={intakeRoots}
              onChange={(next) => setPaths('LIBRARY_INTAKE_ROOTS', next)}
              placeholder="/music/incomming"
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
              {auditMode ? (
                <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 p-4 space-y-2">
                  <Label>Audit mode stays read-only</Label>
                  <p className="text-xs text-muted-foreground">
                    PMDA will index, analyze, and enrich the database view, but it will not publish a clean library, move dupes, move incompletes, or refresh players automatically.
                  </p>
                  <p className="text-[11px] text-muted-foreground">
                    Admins can still trigger explicit manual actions later from the existing review surfaces.
                  </p>
                </div>
              ) : (
                <div className="rounded-lg border border-border/60 bg-background/30 p-4 space-y-2">
        ...
        <div className="rounded-xl border border-border/60 bg-background/20 p-4 space-y-3">
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-2 text-sm">
            ...
            <div>
              ...
              <span className="text-foreground">
                {(mode === 'inplace' || mode === 'audit') ? (sourceRoots[0] || servingRoot || 'current library folders') : (servingRoot || 'not configured yet')}
              </span>
            </div>
            <div>
            ...
            <div>
              <span className="text-muted-foreground">{auditMode ? 'Automatic writes:' : 'Materialization method:'}</span>{' '}
              <span className="text-foreground capitalize">{auditMode ? 'disabled' : materialization}</span>
            </div>
          </div>
