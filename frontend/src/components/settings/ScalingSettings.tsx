...
export function ScalingSettings({ config, updateConfig }: Props) {
  ...
  const providerRows = useMemo(() => {
    ...
    return ['discogs', 'itunes', 'deezer', 'spotify', 'qobuz', 'tidal', 'lastfm', 'audiodb', 'bandcamp'].map((provider) => ({
      provider,
  ...
  const managedMbIsManaged = managedMbBundle?.mode === 'managed';
  const managedMbCanRepair = managedMbBundle?.mode === 'managed' || managedMbBundle?.mode === 'adopted';
  const managedOllamaModeLabel = managedOllamaBundle?.mode
    || (isManagedBundleReady(managedOllamaBundle) && managedOllamaBundle?.effective_url ? 'adopted/external' : 'absent');
  const mbRuntime = runtime?.musicbrainz;
  const mbMirrorFallback = Boolean(mbRuntime?.configured_mirror_enabled && mbRuntime?.fallback_to_public);
  const mbMirrorHealth = mbRuntime?.mirror_health || managedMbBundle?.health || null;
  const mbMirrorReason = String(mbRuntime?.fallback_reason || mbMirrorHealth?.message || '').trim();
  const mbSearchIndexRepairSuggested = Boolean(
    mbMirrorHealth?.repair_action === 'repair-search-index'
      || mbMirrorHealth?.reason_code === 'search_index_empty'
      || mbMirrorHealth?.reason_code === 'search_and_direct_failed',
  );
        ...
        <div className="grid grid-cols-1 xl:grid-cols-2 gap-4">
          <div className="rounded-xl border border-border/60 bg-background/30 p-4 space-y-4">
            ...
            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
              <Badge variant={runtime?.musicbrainz.mirror_enabled ? 'default' : 'outline'}>
                {runtime?.musicbrainz.mirror_enabled ? 'Mirror active' : mbMirrorFallback ? 'Public fallback' : 'Public MB'}
              </Badge>
            ...
            {mbMirrorFallback ? (
              <div className="flex items-start gap-2 rounded-lg border border-warning/30 bg-warning/10 px-3 py-2 text-xs text-warning-foreground">
                ...
                <span>
                  MusicBrainz local mirror is configured, but PMDA is using the public API fallback.
                  {mbMirrorReason ? ` ${mbMirrorReason}.` : ''}
                </span>
              </div>
            ) : null}
          </div>
          ...
          <div className="rounded-xl border border-border/60 bg-background/30 p-4 space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              ...
              <NumericSetting
                label="Worker count"
                help="Set 0 for automatic sizing from the current CPU budget."
              ...
              <NumericSetting
                label="Batch size"
                help="Set 0 for automatic batch sizing derived from the effective worker count."
            ...
            <div className="flex flex-wrap gap-2 text-xs text-muted-foreground">
              ...
              <Badge variant={runtime?.metadata_workers.worker_count_mode === 'manual' ? 'outline' : 'default'}>
                Workers: {runtime?.metadata_workers.worker_count_mode === 'manual' ? 'Manual override' : 'Automatic'} ({runtime?.metadata_workers.effective_worker_count ?? runtime?.metadata_workers.worker_count ?? 0})
              </Badge>
              <Badge variant={runtime?.metadata_workers.batch_size_mode === 'manual' ? 'outline' : 'default'}>
                Batch: {runtime?.metadata_workers.batch_size_mode === 'manual' ? 'Manual override' : 'Automatic'} ({runtime?.metadata_workers.effective_batch_size ?? runtime?.metadata_workers.batch_size ?? 0})
              </Badge>
            </div>
          </div>
        </div>
        ...
        <div className="rounded-xl border border-border/60 bg-background/30 p-4 space-y-4">
          <div className="flex items-center justify-between gap-3">
            <div className="space-y-1">
              ...
              <p className="text-xs text-muted-foreground">
                Shared cache and rate limiting for provider traffic. This is the safe way to accelerate metadata lookups without flooding Discogs, iTunes, Deezer, Last.fm or Bandcamp.
              </p>
            </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-3 gap-3">
            ...
            <NumericSetting
              label="iTunes RPM"
              help="Shared iTunes / Apple Music budget per minute."
              value={config.PROVIDER_GATEWAY_ITUNES_RPM}
              min={1}
              max={1200}
              onChange={(value) => updateConfig({ PROVIDER_GATEWAY_ITUNES_RPM: value })}
            />
            <NumericSetting
              label="Deezer RPM"
              help="Shared Deezer budget per minute."
              value={config.PROVIDER_GATEWAY_DEEZER_RPM}
              min={1}
              max={1200}
              onChange={(value) => updateConfig({ PROVIDER_GATEWAY_DEEZER_RPM: value })}
            />
            <NumericSetting
              label="Spotify RPM"
              help="Shared Spotify budget per minute."
              value={config.PROVIDER_GATEWAY_SPOTIFY_RPM}
              min={1}
              max={1200}
              onChange={(value) => updateConfig({ PROVIDER_GATEWAY_SPOTIFY_RPM: value })}
            />
            <NumericSetting
              label="Qobuz RPM"
              help="Shared Qobuz budget per minute."
              value={config.PROVIDER_GATEWAY_QOBUZ_RPM}
              min={1}
              max={1200}
              onChange={(value) => updateConfig({ PROVIDER_GATEWAY_QOBUZ_RPM: value })}
            />
            <NumericSetting
              label="TIDAL RPM"
              help="Shared TIDAL budget per minute."
              value={config.PROVIDER_GATEWAY_TIDAL_RPM}
              min={1}
              max={1200}
              onChange={(value) => updateConfig({ PROVIDER_GATEWAY_TIDAL_RPM: value })}
            />
            <NumericSetting
              label="Last.fm RPM"
            ...
            />
            <NumericSetting
              label="TheAudioDB RPM"
              help="Shared TheAudioDB budget per minute."
              value={config.PROVIDER_GATEWAY_AUDIODB_RPM}
              min={1}
              max={1200}
              onChange={(value) => updateConfig({ PROVIDER_GATEWAY_AUDIODB_RPM: value })}
            />
            <NumericSetting
              label="Bandcamp RPM"
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-3">
            <div className="rounded-xl border border-border/60 bg-background/30 p-4 space-y-3">
              ...
              {managedMbIsManaged ? (
                ...
                <div className="rounded-lg border border-border/60 bg-background/20 px-3 py-3 text-[11px] text-muted-foreground">
                  This MusicBrainz runtime is adopted or external. PMDA uses it for lookups{managedMbCanRepair ? ' and can run manual search-index repair when Docker is available.' : ', but it cannot schedule Docker-side reindex maintenance for it.'}
                </div>
              )}
              <div className="flex flex-wrap gap-2">
                ...
                <Button type="button" variant="outline" size="sm" onClick={() => void runManagedAction('musicbrainz_local', 'retry-update')} disabled={managedBusy !== '' || !managedMbIsManaged}>
                  Run update now
                </Button>
                <Button type="button" variant={mbSearchIndexRepairSuggested ? 'default' : 'outline'} size="sm" onClick={() => void runManagedAction('musicbrainz_local', 'repair-search-index')} disabled={managedBusy !== '' || !managedMbCanRepair}>
                  {managedBusy === 'musicbrainz_local:repair-search-index' ? <Loader2 className="mr-2 h-3.5 w-3.5 animate-spin" /> : null}
                  Repair search index
                </Button>
                <Button type="button" variant="outline" size="sm" onClick={() => void runManagedAction('musicbrainz_local', 'restart')} disabled={managedBusy !== '' || !managedMbIsManaged}>
                  Restart
                </Button>
                <Button type="button" variant="outline" size="sm" onClick={() => void runManagedAction('musicbrainz_local', 'reset')} disabled={managedBusy !== '' || !managedMbIsManaged}>
                  Reset
                </Button>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-border/60 bg-background/30 p-4 space-y-3">
              ...
              <p className="text-xs text-muted-foreground">
                {managedOllamaBundle?.phase_message || 'No managed Ollama runtime initialized yet.'}
                {' '}Bootstrap first adopts a reachable Docker Ollama service, then pulls missing configured models; it only creates PMDA’s own Ollama container when no usable service is found.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-muted-foreground">
                <div>Mode: <span className="text-foreground">{managedOllamaModeLabel}</span></div>
